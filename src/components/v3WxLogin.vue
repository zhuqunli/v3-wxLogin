  ...
  export default {
    setup(props) {
      const setSrc = computed(() => {
        return `https://open.weixin.qq.com/connect/qrconnect?appid=${props.appid}&scope=${props.scope}&redirect_uri=${props.redirect_uri}&state=${props.state}&login_type=${porps.login_type}&style=${props.login_type}&style=${props.theme}&self_redirect=${props.self_redirect}&href=${props.href}`
      })
      return { setSrc }
    },
    props: {
